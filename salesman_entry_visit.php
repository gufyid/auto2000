...
$cust=$_GET['cust'];
myConn();
$kunjungan=mysql_fetch_array(mysql_query("select * from t_kunjungan where id='$_GET[eid]'"));
$customer=mysql_fetch_array(mysql_query("select nama from t_customer where kode='$kunjungan[customer]'"));
?>
...
<link rel="stylesheet" href="./styles/1.10.4/jquery-ui.css" type="text/css" media="screen" title="core css file" charset="utf-8">
...
<script src="./libs/1.10.4/jquery-ui.js"></script>
...
$q=mysql_query("select kode,nama from t_customer where salesman='$ARSESS[1]' order by nama");
$nama=array();
while($dtq=mysql_fetch_array($q)){
array_push($nama,array('label'=>$dtq[nama],'value'=>$dtq[kode]));
}
...
source: function( request, response ) {
               var matcher = new RegExp( "^" + $.trim($.ui.autocomplete.escapeRegex( request.term )), "i" );
		...
		//console.log(ui.item);
                $("#kode_customer").val(ui.item.value);
...
<div id="ka"><input type="text" class="small" name="tgl" id="tgl" readonly="readonly" value="<?php echo $kunjungan['tgl'];?>"/></div>
<div id="divider"></div>
<div id="divider"></div>

<input type="hidden" name="kodeh" value="<?php echo $_GET['x'];?>">
<input type="hidden" name="kunjunganh" value="<?php echo $kunjungan['id'];?>">

<div id="ki">Customer</div>
<div id="ka"><input name="kode_customer" id="kode_customer" value="<?php echo "$customer[kode]";?>"/><br />
<input name="nama_customer" id="nama_customer" value="<?php echo "$customer[kode]";?>"/></div>
...
<div id="ki">7Step</div>
<div id="ka"><select name="sevenstep" id="sevenstep" onchange="process_7step()"/>
<option value="" selected="selected">Pilih</option>
<?php
...
<div id="ki">Keterangan</div>
<div id="ka"><textarea name="keterangan" id="keterangan" cols="30" rows="10"><?php echo $kunjungan['keterangan'];?></textarea></div>
<div id="divider"></div>
<div id="divider"></div>
...
//	echo $_POST['loss'];
	...
	$customer=$cust1[1];
			...
			echo "<meta http-equiv=refresh content=2;url=?c=salesman&sub=entry&action=visit>";		
				...
				if($_POST['sevenstep']=='7'){
echo "<meta http-equiv=refresh content=2;url=?c=salesman&sub=entry&action=after&cust=$customer&akhir=$akhir>";		
}
				}
				}
}
