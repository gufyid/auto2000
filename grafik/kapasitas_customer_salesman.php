...
$spvku=mysql_fetch_array(mysql_query("select * from t_supervisor where kode='$spv'"));
$saleku=mysql_fetch_array(mysql_query("select * from t_salesman where kode='$salesman'"));
$salesname=str_replace("'","",$saleku['nama']);
myConn();
if($user=='1'){
if($salesman=='all'){
...
$strXML = "<graph caption='Data Kapasitas Customer' subCaption='Auto 2000 Cabang Waru' xAxisName='Score' yAxisName='Persentasi'>";
}else{
...
$strXML = "<graph caption='Data Kapasitas Customer' subCaption='Salesman : $salesname' xAxisName='Score' yAxisName='Persentasi'>";
}
...
$strXML = "<graph caption='Data Kapasitas Customer' subCaption='Supervisor : $spvku[nama]' xAxisName='Score' yAxisName='Persentasi'>";
...
$jum=mysql_fetch_array(mysql_query("select count(*) as jumlah from t_customer where score='$warna[$i]' and salesman='$salesman'"));
}
}else{
if($salesman=='all'){
$jum=mysql_fetch_array(mysql_query("select count(*) as jumlah from t_customer a
left join t_salesman b on b.kode=a.salesman 
left join t_supervisor c on c.kode=b.spv where a.score='$warna[$i]' and c.kode='$spv'"));
}else{
$jum=mysql_fetch_array(mysql_query("select count(*) as jumlah from t_customer a
left join t_salesman b on b.kode=a.salesman 
left join t_supervisor c on c.kode=b.spv where a.score='$warna[$i]' and c.kode='$spv' and a.salesman='$salesman'"));
